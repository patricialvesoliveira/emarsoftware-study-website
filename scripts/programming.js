...
var robot = new Robot(Config.getURLParameter('robot') ? Config.getURLParameter('robot') : 0);
...
function initialize() {
  ...
  Robot.initialize(undefined, username);
...
async function runDefaultProgram(program, robotId) {
  Robot.initialize(undefined, await firebase.auth().currentUser.displayName);
...
function collectAllProgramsFromUsers(snapshot) {
  ...
  for (let i=0; i<nRobotsWithPrograms; i++){
    ...
    let dir = 'study_users/' + username + "/robots/" + robotId + "/";
...
function deleteProgram() {
  if (isMyProgram && myPrograms!=null) {
    ...
    if (confirmation) {
      ...
      var dir = 'study_users/' + username + '/robots/' + selectedRobotId + '/programs/';
      var dbRef = firebase.database().ref(dir);
      dbRef.set(temp);
      resetProgram();
      setSelectedRobot(selectedRobotId);
