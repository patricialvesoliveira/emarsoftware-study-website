...
class Robot {
  constructor(robotId, apiDiv) {
    ...
    Robot.initialize = function (apiDiv, username) {
      console.log('robot initialize', username);
      if (apiDiv != undefined)
        Robot.apiDiv = apiDiv;
      Robot.username = username;
      var dbRefAPI = firebase.database().ref(
        'study_users/' + username + '/robots/' + Robot.robotId + '/customAPI/');
      dbRefAPI.on("value", Robot._updateRobotAPI);
      console.log('Bruh2');
      var dbRefInput = firebase.database().ref(
        'study_users/' + username + '/robots/' + Robot.robotId + '/inputs/');
    ...
    */
    ...
    Robot._requestRobotAction = function (actionName, params) {
      requestRobotActionHelper(actionName, params);
    };

    async function requestRobotActionHelper(actionName, params) {
      let username = await firebase.auth().currentUser.displayName;
      var dbRef = firebase.database().ref('study_users/' + username + "/robots/" + Robot.robotId
        + "/actions/" + actionName + "/");
      dbRef.update(params);
    }

    Robot._requestRobotState = function (stateName, newState) {
      requestRobotStateHelper(stateName, newState);
    };

    async function requestRobotStateHelper(stateName, newState) {
      let username = await firebase.auth().currentUser.displayName;
      console.log('request robot state', username, Robot.robotId);
      var dbRef = firebase
        .database()
        .ref(
          'study_users/' + username + '/robots/' + Robot.robotId + '/state/'
        );
      var updates = {};
      updates[stateName] = newState;
      console.log(Robot.robotId, newState);
    ...
    this.setScreen = function (screenIndex) {
      ...
      console.log(Robot.bellyScreens);
