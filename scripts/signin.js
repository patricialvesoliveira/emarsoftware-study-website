...
function signIn() {
  ...
  console.log(login);
  console.log(login.length, participants.includes(login));
  if (login.length != 0 && participants.includes(login)) {
    if (Database.app.auth().currentUser.displayName != login) {
      firebase
        ...
        .then(() => {
          firebase
            .database()
            .ref('study_users/' + login)
            .update({ group_id: ids[login] })
            .then(() => {
              console.log(login);
    ...
    alert(
      'Please enter your login ID as specified in your instructional kit & try again. Example: firstname_lastinitial'
    );
  }
}
